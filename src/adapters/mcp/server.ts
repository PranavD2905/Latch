...
export function createServer(deps: AppDeps): McpServer {
  ...
  server.registerTool(
    ...
    {
      description:
        "Read the merchant's current versioned policy: deposit amount, the cancellation ladder (retention percentage by hours-before-appointment), and hold TTL. Always call this before confirm_with_deposit and pass back the returned policyVersion as acknowledgedPolicyVersion.",
      inputSchema: {},
    },
  ...
  server.registerTool(
    ...
    {
      description:
        "Read-only status for one booking: its lifecycle status, deposit/authorisation state, and hold expiry if still held. No gate, no money moved. Always safe to call, including as a retry after a prior tool call timed out without a response — this reports what actually happened server-side rather than what the caller assumes happened. If confirm_with_deposit is waiting on a payment, the result's pendingPayment field lists what's still outstanding and what's already done, each with a plain-language label and amount (e.g. \"₹400 remaining-balance hold — charged after your appointment is marked complete\") — read these out in normal sentences, never as raw field names or leg identifiers. This tool alone never finishes a booking, even if everything shows as done — only confirm_with_deposit actually confirms it, so if pendingPayment.outstanding is empty here, call confirm_with_deposit next rather than telling the user it's confirmed.",
      inputSchema: {
  ...
  server.registerTool(
    ...
    {
      description:
        "Confirm a held booking. Depending on the merchant's policy this captures a deposit and/or registers a session-complete mandate for the rest of the price — only the legs the policy actually calls for; a merchant with no deposit configured has no deposit leg at all, for example. Requires a live, unexpired hold and the current policy version acknowledged — call get_policy first and pass its policyVersion as acknowledgedPolicyVersion. Since this account can't take payment server-to-server, a human has to actually pay, so this call returns FAST rather than waiting.\n\n" +
        "If status is 'PENDING': give the user the single payUrl verbatim as a link (one page covers every applicable leg, however many there are). Describe what it covers using each item in outstanding — its label already reads naturally, e.g. \"a ₹300 deposit for your booking\" and \"a ₹400 remaining-balance hold, charged after your appointment is marked complete\" — never mention field names like 'leg' or 'outstanding' or show raw JSON. Then STOP and ask the user to say once they've paid. Do not call this tool again on a timer, and do not retry it speculatively — only call it again after the user tells you they've paid, or explicitly asks you to check. When you do check again: use the SAME bookingId and idempotencyKey (cheap re-check, not a re-charge, safe to call repeatedly). If it's still PENDING, some items may now be missing from outstanding because they're done — tell the user in plain language exactly which ones are still left (e.g. \"the deposit went through, but the remaining-balance hold hasn't been authorised yet\") and ask again; do not assume everything succeeded just because you're checking.\n\n" +
  ...
  server.registerTool(
    ...
    {
      description:
        'Cancel a confirmed booking as the customer. The cancellation ladder applies: how much of the deposit is retained vs. refunded is computed from how far ahead of the appointment the cancellation happens, at the server clock — never a time the caller claims. Releases the session-complete mandate; leaves no live authority against the booking.',
      inputSchema: {
        bookingId: z.string().describe('bookingId returned by hold_slot'),
        idempotencyKey: z.string(),
      },
