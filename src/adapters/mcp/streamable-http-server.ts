 ...
 *
 * Deliberately unauthenticated. docs' own thesis (agentic-services-
 * transactability-brief.md, and slice-7.md itself) is that a third-party
 * agent can transact with this merchant with no partnership and no
 * integration deal — gating `/mcp` behind a bearer token would contradict
 * that directly. `merchant-api` and the audit-trail viewer keep their own
 * narrow tokens; this route is the one meant to be genuinely open. Real
