...
| **No merchant-wide hold-rate ceiling, only per-agent-per-merchant (dev-logs/016)** | `holdRateLimitPerMinute` bounds one `agentId`; a large number of *distinct* hostile agent ids, each staying under its own ceiling, could still collectively spam one merchant's calendar | Considered and deliberately not built this session: a merchant-wide ceiling is new policy surface (another number to tune, another gate to explain) for a threat model — many independently-run hostile agents coordinating against one merchant — with no evidence it's the actual risk here, versus one agent's own re-holding, which is what dev-logs/014 built for and is real |
