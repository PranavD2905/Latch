...
| `Policy` | Deposit rule + cancellation ladder. **Versioned.** | The authority every money action cites (B2) |
...
follow-up, dev-logs entry). `deposit` is optional, and the session-complete mandate is
`service.price_paise - (deposit.amount_paise ?? 0)` — zero when the service is priced exactly at the
deposit. So a booking carries one or two payment legs depending entirely on what the merchant
configured, and `confirm_with_deposit` offers a pay link for exactly those. A merchant with no deposit
has a single leg: the session-complete hold for the full price. Absent is the only way to express "no
deposit" — an explicit `0` is rejected by `validatePolicyInput`, which is what makes a ₹0 Razorpay order
structurally impossible rather than merely avoided.

**No separate no-show fee — the ladder's own floor tier is the recovery mechanism.** Latch originally
modelled the no-show as its own authorisation-and-charge leg (`AUTHORIZATION_HELD`/`charge_no_show`,
Slice 4). Market research done after that slice shipped found Indian merchants don't actually charge a
no-show fee the way this modelled it — a post-hoc debit against a stored card is a Western pattern.
What Indian salons and clinics do instead is take a deposit and forfeit it on a late cancellation or
no-show, exactly what this ladder's `hours_before: 0` floor tier already expresses. So the no-show leg
was removed rather than kept as unused surface: the deposit, already collected at confirm time, *is*
the no-show recovery — nothing further needed building. See the dev log for this removal, and
`04-features-and-limitations.md` for what that means for a booking nobody ever cancels or completes.
...
```
  ...
  (a patient who never shows and is never cancelled just stays CONFIRMED —
   the deposit already collected at confirm time is the recovery; see the
   note below the ladder in §2)
```
...
**Rule 3 — `NO_SHOW_ELIGIBLE`/`NO_SHOW_CHARGED` are historical-only.** Slice 4 originally built a
two-step no-show charge here — time passing made a charge *permissible* (`NO_SHOW_ELIGIBLE`), an
explicit, unforgeable merchant action made it *happen* (`charge_no_show` → `NO_SHOW_CHARGED`). Removed
(see the dev log for that removal): the no-show-eligibility background worker and `charge_no_show` are
both gone, so no live code path can reach either state again. Both event types remain in the domain's
event union purely so `fold()` and the audit-trail viewer can keep correctly replaying/rendering a
pre-removal booking's own recorded history — the `events` table is the source of truth and is never
rewritten, so a booking that really was charged for a no-show before this removal must keep showing
that as its true state.
...
| `AUTHORIZATION_HELD` | — | **Historical-only** (see the no-show removal note below) — card manual capture token created, ceiling recorded, for the removed no-show leg |
| `SESSION_COMPLETE_AUTHORIZATION_HELD` | — | A card manual-capture token created alongside the deposit — `service.pricePaise - policy.depositAmountPaise`, frozen at confirm time (added with the session-complete charge feature; see the note after this table) |
| `SESSION_COMPLETE_AUTHORIZATION_RELEASED` | — | The session-complete mandate abandoned unused — a decline or customer cancellation made it moot |
...
| `AUTHORIZATION_RELEASED` | — | **Historical-only** — the removed no-show authorisation, abandoned unused |
| `ALTERNATIVES_OFFERED` | — | Replacement slots computed and pushed |
| `AUTHORIZATION_LAPSED` | — | **Historical-only** — the removed no-show authorisation's 5-day window expiring unused |
| `NO_SHOW_ELIGIBLE` | — | **Historical-only** — start + grace elapsed, for the removed no-show leg |
| `NON_ATTENDANCE_MARKED` | — | **Historical-only** — merchant API marked non-attendance, for the removed no-show leg |
| `NO_SHOW_CHARGED` | **in** | **Historical-only** — debit against the removed no-show authorisation succeeded |
...
★ the B5 failure path. `SESSION_COMPLETE_AUTHORIZATION_RELEASED` is also part of it — decline releases
whichever authorisation legs the booking actually has. (`AUTHORIZATION_RELEASED`, the no-show leg's own
release event, used to appear here too before that leg was removed — see the dev log for that removal;
a pre-removal decline's five-event transaction still shows it, historically.)
...
every leg this booking's policy actually calls for (see §2 — one or both of `deposit` /
`session_complete_authorization`; `no_show_authorization` was a third possible value before that leg
was removed — see the dev log for that removal — and remains a valid historical value for a
pre-removal booking's own recorded legs), done or not: the single `/pay/:bookingId`
...
elapsed time (e.g. a cancellation ladder tier boundary) — those rows land for real in the shared dev database with
...
feature.** `fold()` (`src/domain/fold.ts`) deliberately treats
`SESSION_COMPLETE_AUTHORIZATION_HELD`/`_RELEASED`/`_LAPSED` as no-ops: `BookingState`, the pure fold's
return type, carries a single `authorizationId` field — historically the no-show leg's, now unused by
any live code path (see the no-show removal note earlier in this section). The session-complete leg's
authorisation id, amount, and expiry live on the live Postgres projection's own dedicated columns
(`sessionCompleteAuthorizationId` and friends, migration `0013`), set directly by
`confirm-with-deposit.ts`/`mark-session-complete.ts`, never derived from a replay. This is not an
oversight — a pure `fold()` over the event log is the reference domain model `docs/03-domain-model.md`
describes and `fold.test.ts` exercises directly, but it is not, and was never meant to be, the thing any
command handler actually reads at runtime. The live projection (`BookingSnapshot`,
`src/ports/event-store.ts`) is a superset of `fold()`'s `BookingState`, carrying whatever additional
operational bookkeeping (`holdExpiresAt`, `agentId`, the session-complete authorisation leg's full
detail) a command handler's own gate needs without a full replay per call — see dev-logs/010 for why
that split was made in the first place. The event still lands in the trail, in order, exactly where a
real replay would show it; only `fold()`'s own projected fields stay narrower than the live one's.
...
```jsonc
{
  ...
  "gate": {                                      // B4 — what permitted it
    "cleared": ["merchant_marked_session_complete"],
    "evidence": { "started_at": "...", "marked_by": "merchant" }
  },
  "bound": {                                     // B3 — the ceiling, and who holds it
    "ceiling_paise": 50000,                      // the authorised amount itself
    "enforced_by": "payment_rail",               // ← not "latch"
    "headroom_after_paise": 0                    // authorised at exactly the mandate: no slack
  },
...
Read that object and you can reconstruct the entire justification for ₹500 leaving someone's account
...
**`rail` appears on `SESSION_COMPLETE_CHARGED` (and historically `NO_SHOW_CHARGED`) only, not on every
money event.** Narrower than dev-log 005's original phrasing ("every money event carries `rail`").
`DEPOSIT_CAPTURED` / `RETENTION_APPLIED` / `REFUND_ISSUED` always settle through the same
`PaymentProvider` Checkout capture regardless of which `PaymentRail` is active — there is no rail choice
for them to name. `SESSION_COMPLETE_AUTHORIZATION_HELD` / `_RELEASED` / `_LAPSED` (and historically
`AUTHORIZATION_HELD` / `AUTHORIZATION_RELEASED` / `AUTHORIZATION_LAPSED`, for the removed no-show leg)
carry it too, since they're the events that name which rail is holding the authority in the first place.
...
The `Nothing` row is the interesting one. Most API errors tell a caller how to succeed. This one tells
it that no path exists — which is what a *bound* means, as opposed to a *validation error*.

(`NOT_YET_ELIGIBLE`, `MERCHANT_ACTION_REQUIRED`, and `AUTHORIZATION_EXPIRED` were the removed no-show
leg's own refusal codes — gone along with `charge_no_show`; see the dev log for that removal.)
...
Thu 14:03:48  POLICY_ACKNOWLEDGED   ladder v4  (≥48h free / 12–48h 50% / <12h 100%)
                                    deposit ₹300 · service ₹800 · session-complete ceiling ₹500
...
Thu 14:04:03  SESSION_COMPLETE_AUTHORIZATION_HELD    pay_Auth991  authorised ₹500  lapses in 5d
                                    bound: ₹500 — the authorised amount IS the ceiling
...
Wed 11:20:34  SESSION_COMPLETE_AUTHORIZATION_RELEASED       pay_Auth991 abandoned — never captured
...
`SESSION_COMPLETE_AUTHORIZATION_RELEASED` → `ALTERNATIVES_OFFERED`) match exactly, in order, in one
transaction. `SESSION_COMPLETE_AUTHORIZATION_HELD`/`_RELEASED` themselves are proven against
`FakePaymentRail` — which reproduces `ManualCaptureRail`'s real behaviour exactly, including the item-7
ceiling refusal — and against real Razorpay for order creation
(`manual-capture-rail.live.integration.test.ts`); a real authorised payment landing and being captured
needs a human at Checkout (dev-logs/006/007), which is carried forward rather than blocking Slice 4
(`dev-logs/009`).

**What happens if the patient never shows up at all, now that the no-show leg is removed** (see the dev
log for that removal): nothing automatic. The booking simply stays `CONFIRMED` — the deposit already
collected at confirm time is the recovery, exactly the deposit-forfeiture mechanism Indian merchants
already use, and there is no further action for the system to take unless the merchant marks the
session complete (capturing the session-complete mandate) or the customer cancels (running the
ladder). This slice used to end here with a two-independent-facts no-show charge
(`NON_ATTENDANCE_MARKED` → `charge_no_show` → `NO_SHOW_CHARGED`, gated on `NOT_YET_ELIGIBLE` /
`MERCHANT_ACTION_REQUIRED`); a booking confirmed before the removal may still carry that history, and
`fold()` still replays it correctly, but no new booking can reach it.
